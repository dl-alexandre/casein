defmodule CaseinWeb.WorkspaceLive.Show.SessionBarTest do
  ...
  describe "sessions_sidebar/1 focus + quiet chrome" do
    ...
    test "renders the pinned cross-workspace Needs-you strip with deep-link rows" do
      current =
        SessionBarVM.session_tabs([
          agent_info("blocked", "tmux-blocked")
          |> Map.put(:metadata, %{
            windows: [%{id: "@1", agent_state: :blocked}],
            agent_state_messages: %{"@1" => "needs your approval"}
          }),
          agent_info("calm", "tmux-calm")
        ])
      ...
      other =
        SessionBarVM.session_tabs([
          agent_info("remote-done", "tmux-remote")
          |> Map.put(:metadata, %{windows: [%{id: "@1", agent_state: :done}]})
        ])

      needs_you =
        SessionBarVM.needs_you_strip(current, "ws-1",
          sidebar_ws_sessions: %{"ws-2" => other},
          summaries: [%{id: "ws-1", name: "alpha"}, %{id: "ws-2", name: "beta"}]
        )
      ...
      html =
        render_component(&SessionBar.sessions_sidebar/1,
          ...
          needs_you: needs_you,
          active_id: "agent_blocked",
          session_tabs: current
        )

      assert html =~ ~s(id="sessions-needs-you-ws-1")
      assert html =~ "data-needs-you-strip"
      # Current-workspace blocked row attaches in place and shows its message.
      assert html =~ ~s(data-needs-you-reason="blocked")
      assert html =~ ~s(phx-value-session-id="agent_blocked")
      assert html =~ "needs your approval"
      # Cross-workspace done row navigates and is tagged with its workspace.
      assert html =~ ~s(data-needs-you-reason="completed")
      assert html =~ "beta"
    end

    test "omits the Needs-you strip entirely when nothing needs attention" do
      tabs = SessionBarVM.session_tabs([agent_info("calm", "tmux-calm")])
      ...
      refute html =~ "data-needs-you-strip"
    end
