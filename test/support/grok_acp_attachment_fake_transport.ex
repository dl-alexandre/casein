defmodule Casein.Test.GrokACPAttachmentFakeTransport do
  ...
  def start(owner, opts) do
    ...
    send(test_pid, {:grok_acp_attachment_transport_started, owner, opts})
    {:ok, %{owner: owner, test_pid: test_pid}}
  end

  @impl true
  def write(%{owner: owner, test_pid: test_pid}, data) do
    send(test_pid, {:grok_acp_attachment_transport_write, owner, IO.iodata_to_binary(data)})
    :ok
  end

  @impl true
  def stop(%{owner: owner, test_pid: test_pid}) do
    send(test_pid, {:grok_acp_attachment_transport_stopped, owner})
    :ok
  end
end
