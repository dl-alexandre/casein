defmodule TmuxCtl.Test.FakeRunner do
  ...
  defp panes_output(session, opts \\ []) do
    ...
    |> Enum.map_join("\n", fn p ->
      prefix <>
        "#{p.window_id}|#{p.id}|#{p.index}|#{active(p.active)}|#{p.left}|#{p.top}|#{p.width}|#{p.height}|#{p.current_command}|#{p.activity}|#{bell(p.bell)}|#{p.activity}|#{active(p.activity_flag)}|#{bell(p.bell)}|#{active(p.unseen_changes)}|#{p.current_path}"
    end)
  end
