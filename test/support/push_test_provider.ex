defmodule Casein.Push.TestProvider do
  ...
  `:casein, :push_test_pid` as `{:pushed, token, platform, notification}` so
  ...
  def push(token, platform, notification) do
    case Application.get_env(:casein, :push_test_pid) do
      pid when is_pid(pid) -> send(pid, {:pushed, token, platform, notification})
      _ -> :ok
    end

    case Application.get_env(:casein, :push_test_response, :ok) do
