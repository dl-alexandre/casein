defmodule DevIDE.Runtimes.PreviewKillerTest do
  @moduledoc """
  Unit tests for `DevIDE.Runtimes.PreviewKiller`.

  The public `kill/1` seam routes through `Application.get_env(:dev_ide,
  ...
  alias DevIDE.Runtimes.PreviewKiller
  ...
  setup do
    previous = Application.get_env(:dev_ide, :runtime_preview_killer)
  ...
  describe "kill/1 public API" do
    ...
    test "delegates to the configured Behaviour impl via :runtime_preview_killer" do
      Application.put_env(:dev_ide, :runtime_preview_killer, FakeKiller)
    ...
    test "defaults to Default when env is unset" do
      Application.delete_env(:dev_ide, :runtime_preview_killer)
  ...
  describe "Default.kill/1 registry + port branches" do
    setup do
      Application.put_env(:dev_ide, :runtime_preview_killer, PreviewKiller.Default)
      :ok
    end
  ...
  defp restore_env(nil), do: Application.delete_env(:dev_ide, :runtime_preview_killer)
  defp restore_env(val), do: Application.put_env(:dev_ide, :runtime_preview_killer, val)
end
