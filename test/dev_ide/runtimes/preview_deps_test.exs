defmodule DevIDE.Runtimes.PreviewDepsTest do
  @moduledoc """
  Unit tests for `DevIDE.Runtimes.PreviewDeps` — the core-side impl of
  `DevIDE.Previews.Deps.Runtimes`.

  Mirrors the callback surface in `DevIDE.Previews.Deps.Test.Fakes.Runtimes` and
  asserts each of the four public callbacks reaches `DevIDE.Runtimes` /
  `DevIDE.Runtimes.PreviewLauncher` (thin pure delegation; no local logic).
  """

  use DevIDE.TestCase, async: false

  alias DevIDE.Previews.Deps.Runtimes, as: RuntimesBehaviour
  alias DevIDE.Runtimes
  alias DevIDE.Runtimes.{PreviewDeps, PreviewLauncher, Runtime}
  ...
  describe "behaviour contract" do
    test "implements every DevIDE.Previews.Deps.Runtimes callback" do
  ...
  describe "delegation to DevIDE.Runtimes / PreviewLauncher" do
    ...
    test "ensure_preview_server_started/1 reaches PreviewLauncher.ensure_started/1" do
      ...
      after
        case previous do
          nil -> Application.delete_env(:dev_ide, :runtime_preview_launcher_enabled)
          value -> Application.put_env(:dev_ide, :runtime_preview_launcher_enabled, value)
        end
      end
    end
  end
