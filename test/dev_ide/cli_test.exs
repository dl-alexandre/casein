defmodule DevIDE.CLITest do
  # async: false — the runtimes memory adapter is a shared named process.
  ...
  describe "runtimes subcommands" do
    ...
    test "expire errors (not crashes) for an unknown runtime id" do
      assert {:error, "runtime not found: rt-missing"} =
               CLI.run(["runtimes", "expire", "rt-missing"])
    end

    test "cleanup errors (not crashes) for an unknown runtime id" do
      assert {:error, "runtime not found: rt-missing"} =
