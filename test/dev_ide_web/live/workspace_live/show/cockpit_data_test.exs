defmodule DevIdeWeb.WorkspaceLive.Show.CockpitDataTest do
  @moduledoc """
  Direct unit coverage for the DB-free public functions of
  `DevIdeWeb.WorkspaceLive.Show.CockpitData`.
  ...
  setup do
    prev = Application.get_env(:dev_ide, :lan_path_root)
    Application.put_env(:dev_ide, :lan_path_root, "/lanroot")

    on_exit(fn ->
      if prev do
        Application.put_env(:dev_ide, :lan_path_root, prev)
      else
  ...
  describe "lan_path_error/2" do
    ...
    test "returns reserved title and message for :reserved_prefix" do
      ...
      assert error.message == "path is reserved by DevIDE"
      assert error.route_path == "/foo/bar"
      assert error.relative_path == "foo/bar"
      assert error.root_path == "/lanroot"
      assert error.target_path == nil
