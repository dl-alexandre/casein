defmodule Casein.ProposalsNoApplyTest do
  ...
  @read_only_sources [
    "lib/casein/proposals.ex",
    "lib/casein/proposals/adapter.ex",
    "lib/casein/proposals/local_adapter.ex",
    "lib/casein/proposals/unified_diff.ex",
    "lib/casein/proposals/proposal.ex"
  ]
  ...
  test "proposal:* events live in ProposalPanelComponent, gated and never applied in Show" do
    show_src =
      File.read!(Path.expand("lib/casein_web/live/workspace_live/show.ex", File.cwd!()))
    ...
    component_src =
      File.read!(
        Path.expand(
          "lib/casein_web/live/workspace_live/proposal_panel_component.ex",
          File.cwd!()
        )
      )
  ...
  test "the write path lives only in Casein.ProposalApply, gated by Policy" do
    ...
    git_adapter_src =
      File.read!(Path.expand("lib/casein/proposal_apply/git_adapter.ex", File.cwd!()))
