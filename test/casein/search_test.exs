defmodule Casein.SearchTest do
  ...
  setup do
    ...
    prev_adapter = Application.get_env(:casein, :search_adapter)
    Application.put_env(:casein, :search_adapter, Casein.Search.MemoryAdapter)
    Application.delete_env(:casein, :search_memory_results)
    Application.delete_env(:casein, :search_memory_response)
    Application.delete_env(:casein, :search_memory_available)
  ...
  test "returns adapter results", %{root: root} do
    Application.put_env(:casein, :search_memory_results, %{
      "needle" => [
  ...
  test "missing rg surfaced as :rg_missing", %{root: root} do
    Application.put_env(:casein, :search_memory_response, {:error, :rg_missing})
    assert {:error, :rg_missing} = Search.search(root, "needle")
  end

  test "timeout surfaced as :timeout", %{root: root} do
    Application.put_env(:casein, :search_memory_response, {:error, :timeout})
    assert {:error, :timeout} = Search.search(root, "needle")
  end

  test "available?/0 reports adapter availability" do
    Application.put_env(:casein, :search_memory_available, false)
    refute Search.available?()
    Application.put_env(:casein, :search_memory_available, true)
