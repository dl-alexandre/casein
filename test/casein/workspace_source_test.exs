defmodule Casein.WorkspaceSourceTest do
  ...
  describe "capability_detector/0" do
    test "defaults to the configured LocalAdapter" do
      assert WorkspaceSource.capability_detector() == Casein.Agents.LocalAdapter
    end

    test "config override drives the detect_capabilities/2 fallback" do
      prev = Application.fetch_env!(:casein, :capability_detector)
      on_exit(fn -> Application.put_env(:casein, :capability_detector, prev) end)
      Application.put_env(:casein, :capability_detector, StubDetector)
      put_source(EmptySource)

      # The fallback resolves the detector via config — this is the seam that
      # keeps Casein.Agents.LocalAdapter out of WorkspaceSource's compile graph
