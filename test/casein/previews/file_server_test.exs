defmodule Casein.Previews.FileServerTest do
  ...
  setup do
    ...
    prev_clock = Application.get_env(:casein, :file_server_clock)
  ...
  defp seed_workspace! do
    root = Casein.TmpWorkspace.root!("file-server")
  ...
  test "HTTP hits reset the idle timer so an open preview is not reaped mid-use" do
    ...
    idle_ms = 150
    Application.put_env(:casein, :file_server_idle_ms, idle_ms)

    {:ok, clock} = Agent.start_link(fn -> 1_000_000 end)

    Application.put_env(:casein, :file_server_clock, fn ->
      Agent.get(clock, & &1)
    end)
