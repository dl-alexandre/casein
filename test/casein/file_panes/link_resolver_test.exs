defmodule Casein.FilePanes.LinkResolverTest do
  ...
  setup do
    ...
    prev_clock = Application.get_env(:casein, :link_resolver_clock)
    ...
    on_exit(fn ->
      ...
      if prev_clock do
        Application.put_env(:casein, :link_resolver_clock, prev_clock)
      else
        Application.delete_env(:casein, :link_resolver_clock)
      end
    end)
  ...
  describe "caching" do
    ...
    test "entries expire after the TTL", %{root: root} do
      ...
      Application.put_env(:casein, :link_resolver_clock, fn ->
        Agent.get(clock, & &1)
      end)
