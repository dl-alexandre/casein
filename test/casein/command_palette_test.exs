defmodule Casein.CommandPaletteTest do
  ...
  test "FileIndex skips ignored dirs and lists workspace files", %{root: root} do
    ...
    assert "lib/casein/foo.ex" in files
  ...
  test "query ranks file matches and respects limit", %{root: root} do
    ...
    assert Enum.any?(items, &(&1.kind == :file and &1.label == "lib/casein/foo.ex"))
  end

  test "query emits open and open-in-pane variants for each file match", %{root: root} do
    items =
      CommandPalette.query(root, "foo", limit: 50)
      |> Enum.filter(&(&1.label == "lib/casein/foo.ex"))

    by_id = Map.new(items, &{&1.id, &1})

    assert %{
             detail: "Open file",
             payload: %{event: "annotation:open", params: %{"path" => "lib/casein/foo.ex"}}
           } = by_id["file:lib/casein/foo.ex"]

    assert %{
             detail: "Open in pane",
             keywords: keywords,
             payload: %{event: "tree:open_in_pane", params: %{"path" => "lib/casein/foo.ex"}}
           } = by_id["file-pane:lib/casein/foo.ex"]
    ...
    open_idx = Enum.find_index(items, &(&1.id == "file:lib/casein/foo.ex"))
    pane_idx = Enum.find_index(items, &(&1.id == "file-pane:lib/casein/foo.ex"))
