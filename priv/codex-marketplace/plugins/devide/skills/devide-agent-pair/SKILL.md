---
name: devide-agent-pair
description: Operate DevIDE's human-plus-agent tmux layout without colliding with the operator. Use when discovering sessions, selecting the dedicated agent pane, sending commands or text, waiting on another agent, or reading its transcript.
---

# DevIDE agent pairing
...
If the layout lacks an agent pane, ask the operator to apply the built-in `agent_pair` template or use the appropriate DevIDE control to create it.
