#!/usr/bin/env bash
# devide-codex-notify.sh — Codex `notify` hook that reports turn completion to
# DevIDE's terminal MCP endpoint.
#
# Injected by the DevIDE launcher as a per-launch config override
# (`-c notify=["…/devide-codex-notify.sh"]`). Codex invokes the program with a
...
#
# Fire-and-forget like devide-agent-state.sh: missing environment, unmapped
