...
# a cycle (journalctl -u devide-tmp-cleanup.service), then re-run with --apply to
# arm real deletion. Age filtering keeps anything recently in use.
#
# Usage:
#   bash scripts/ensure-devide-tmp-cleanup.sh                 # dry-run (log only)
#   bash scripts/ensure-devide-tmp-cleanup.sh --apply         # arm deletion
#   bash scripts/ensure-devide-tmp-cleanup.sh --user devbox   # run-as user
#   bash scripts/ensure-devide-tmp-cleanup.sh --no-start
#   bash scripts/ensure-devide-tmp-cleanup.sh --disable
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
UNIT_DIR="/etc/systemd/system"
SERVICE="devide-tmp-cleanup.service"
TIMER="devide-tmp-cleanup.timer"
...
chmod 0755 "${ROOT}/scripts/devide-tmp-cleanup.sh"
