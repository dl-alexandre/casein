...
agent_env_load_tmux_session_env() {
  ...
  while IFS= read -r line; do
    ...
    value="${line#*=}"
    case "$key" in
      DEV_IDE_API_TOKEN|DEVIDE_WORKSPACE_ID|DEVIDE_WORKSPACE_NAME|DEVIDE_TMUX_SESSION|DEVIDE_TERMINAL_MCP_URL|DEVIDE_PREVIEW_MCP_URL|DEVIDE_TIDEWAVE_MCP_URL|DEVIDE_PREVIEW_ENV_ID|DEVIDE_CHECKOUT|DEVIDE_AGENT_MCP_HOME|DEVIDE_SCRIPTS|DEVIDE_AGENT_ENV_FILE|DEV_IDE_TERMINAL_SCHEME|DEV_IDE_TERMINAL_PRESET|COLORFGBG|CLAUDE_CONFIG_DIR|CODEX_HOME|PATH)
        if [[ -z "${!key:-}" ]]; then
          export "${key}=${value}"
        fi
        ;;
