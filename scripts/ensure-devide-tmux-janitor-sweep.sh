...
ensure_env_policy() {
  if ! sudo test -f "$ENV_FILE"; then
    log "warning: ${ENV_FILE} missing; skipping idle-threshold env policy"
    return
  fi

  local backup
  backup="${ENV_FILE}.tmux-janitor.$(date -u +%Y%m%d%H%M%S)"
  log "setting tmux janitor idle thresholds in ${ENV_FILE} (backup=${backup})"
...
if [[ "$DISABLE" -eq 1 ]]; then
  ...
  log "removed tmux janitor timer units"
  exit 0
fi
