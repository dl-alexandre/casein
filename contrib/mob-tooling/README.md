...
| `lib/mob/dev_ide/terminal_screen.ex` | native Mob terminal slice — a real `Ghostty.Terminal` rendered as a `Canvas` grid from `cells/1` (the dev_ide terminal, on-device). Staged; drop into a Mob app + add the `:ghostty` dep to run |
