defmodule CaseinWeb.WorkspaceLive.Show do
  ...
  def handle_info({:pty_data, _pane_id, data} = msg, socket) when is_binary(data),
    do: PtyEvents.handle_info(msg, socket)

  def handle_info({:pane_frame, _, _} = msg, socket),
    do: PtyEvents.handle_info(msg, socket)

  def handle_info({:pty_exit, _, _} = msg, socket),
    do: PtyEvents.handle_info(msg, socket)

  def handle_info({:terminal_recovery, %{type: :session_recreated}} = msg, socket),
    do: PtyEvents.handle_info(msg, socket)

  def handle_info({:auto_apply_recovery_template, template_id} = msg, socket)
      when is_binary(template_id),
      do: PtyEvents.handle_info(msg, socket)

  def handle_info({:auto_reattach_pane, _, _} = msg, socket),
    do: PtyEvents.handle_info(msg, socket)
  ...
  def handle_info(:after_mount_side_panels = msg, socket),
    do: AfterMount.handle_info(msg, socket)

  def handle_info(:after_mount_runs = msg, socket),
    do: AfterMount.handle_info(msg, socket)

  def handle_info(:after_mount_agents = msg, socket),
    do: AfterMount.handle_info(msg, socket)
  ...
  def render(assigns) do
    ...
    <.workspace_shell
      ...
      clipboard_drawer_open={@clipboard_drawer_open}
      ...
      codex_selected_thread_id={@codex_selected_thread_id}
      codex_threads={@codex_threads}
      codex_timeline={@codex_timeline}
      ...
      notif_device_stats={@notif_device_stats}
      notif_devices={@notif_devices}
      notif_drawer_open={@notif_drawer_open}
      notif_error={@notif_error}
      notif_info={@notif_info}
      notif_loaded?={@notif_loaded?}
      notif_preferences={@notif_preferences}
      notif_preferences_form={@notif_preferences_form}
