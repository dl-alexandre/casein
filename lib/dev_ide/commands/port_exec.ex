defmodule DevIDE.Commands.PortExec do
  ...
  @doc """
  Spawn `argv` (a list of binaries; first element is the executable) with
  `extra_opts` merged into the `:exec.run/2` option list. Returns
  `{:ok, ref, handle}` or `{:error, reason}`. The handle is accepted by
  `kill/1`.
  """
  ...
  @doc "Kill a process started by `run/3`."
  ...
  # erlexec encodes wait(2) status: low byte 0 means normal exit (code in the
  # high byte); otherwise the process was signalled.
