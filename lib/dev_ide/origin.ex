defmodule DevIDE.Origin do
  @moduledoc """
  Installation-stable identity for this DevIDE origin.
  ...
  def id do
    case configured_string(:origin_id) do
      nil ->
        secret = Application.get_env(:dev_ide, :origin_identity_secret)

        unless is_binary(secret) and byte_size(secret) >= 32 do
          raise "DevIDE origin identity requires a stable endpoint secret_key_base"
        end
  ...
  defp endpoint_base_url do
    case Application.get_env(:dev_ide, DevIdeWeb.Endpoint, []) |> Keyword.get(:url, []) do
  ...
  defp inferred_display_name(nil), do: "DevIDE"
