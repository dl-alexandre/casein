defmodule DevIDE.Agents.PreviewTools.Open do
  @moduledoc "preview_open."

  use Jido.Action,
    name: "preview_open",
    description: "Open a workspace preview. Pass mode to pick the surface.",
    category: "preview",
    tags: ["preview"],
    vsn: "1.0.0",
    schema: [
  ...
  def parameters, do: Tool.object(Helpers.open_unified_props(), [:workspace_id])

  @impl DevIDE.Agents.ToolAction
  def mcp_metadata, do: Helpers.metadata("preview_open")

  @impl Jido.Action
  def run(params, context) do
    workspace = Map.get(context, :workspace, %{})
    Impl.open_unified(workspace, Helpers.to_impl_args(params))
  end
end
