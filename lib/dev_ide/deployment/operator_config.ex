defmodule DevIDE.Deployment.OperatorConfig do
  @moduledoc """
  Validates the data-only contract between DevIDE core and an operator overlay.

  The overlay may provide deployment-specific values in a JSON file selected by
  `DEV_IDE_OPERATOR_CONFIG_FILE`. Keeping the contract data-only prevents a host
  ...
  def load!(path) do
    case load(path) do
      ...
      {:error, reason} ->
        raise ArgumentError,
              "invalid DEV_IDE_OPERATOR_CONFIG_FILE #{inspect(path)}: #{format_error(reason)}"
    end
  end
