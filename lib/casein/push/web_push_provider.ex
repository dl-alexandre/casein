defmodule Casein.Push.WebPushProvider do
  @moduledoc """
  `Casein.Push.Provider` for browser Web Push (RFC 8030/8291/8292), used by the
  installed PWA so a phone gets an "agent needs you" notification while the app
  is backgrounded or closed.

  Encrypts the payload for the stored subscription (`Casein.Push.WebPush.Encryption`,
  verified against the RFC 8291 vector), signs a VAPID header
  (`Casein.Push.WebPush.Vapid`), and POSTs to the endpoint via `Req`.

  Inert until VAPID keys are configured (`config :casein, __MODULE__, …` from
  `CASEIN_VAPID_*`); `configured?/0` reports `{:error, :push_provider_unconfigured}`
  otherwise, so `Casein.Push.ready_for?/1` gates delivery exactly like the other
  providers.
  """
  ...
  defp config do
    case Application.get_env(:casein, __MODULE__) do
