...
| Paste/submit needs "double-Enter" folklore | Do **not** double-Enter yourself. `terminal_paste_agent_text` (`submit: true`) pastes without Enter, settles, then `PaneSubmit` presses Enter **once** and confirms via hook/transcript/screen. Unconfirmed → `submit_not_confirmed` (no auto-retry). Trust `delivery`/`submitted` on the response; `observed: "unknown"` is not failure. Pass explicit `pane` on paste for fleet workers without agent_pair. |
