...
**Runner topology (#816):** only jobs with `runs-on: [self-hosted, devbox]` share the persistent Actions workdir (`…/actions-runner/_work/casein/casein`) and its `clean:false` `_build` (including `native/casein_mob` zigler priv). Today that is **PR gate** + **preview-e2e**. They serialize Mix/deps/compile/test on one host flock (`/tmp/casein-pr-gate.lock` via `scripts/lib/casein-devbox-mix-lock.sh`) and heal incomplete zigler `priv/beam` (missing `sema.zig`) before compile. GitHub concurrency stays **per-PR** (`pr-gate-${{ number }}`, `cancel-in-progress: true`) so same-PR supersession cancels correctly without the #753 global-group trap. **Not on this workdir / not required for gate:** `milcmini` / `testserver` (macOS desktop package), `windows-latest` (Windows desktop), `ubuntu-latest` (pty-tests, deploy-devbox). Extra PR-gate capacity needs separate **Linux** hosts labeled `devbox` (isolated workdirs); more runner processes on one box still correctly serialize on the flock.

Residual Postgres DSM / `ERROR 53100` under `/dev/shm` (segment count / `kernel.shmmni`, not byte capacity) is documented in [`docs/ops-pr-gate-postgres-shm.md`](docs/ops-pr-gate-postgres-shm.md) — attended host sysctl only; do not thrash product PRs. One-time setup per box:
...
**Gate re-trigger trap — do not `gh run rerun` a stale pr-gate run.** PR #760 set pr-gate to **per-PR** concurrency with `cancel-in-progress: true` (group `pr-gate-${{ number || ref }}` plus a host `flock` around the heavy suite). That means re-running an *old* run for a PR cancels that PR's *live* run — real incident: PR #759, re-run of 31292593333 cancelled live 31293820318. Prefer an empty-commit re-trigger (`git commit --allow-empty -m "ci: re-trigger gate" && git push`) or the workflow's `workflow_dispatch` trigger. **Why the concurrency looks like this:** #753 first used one global group (`pr-gate-devbox-self-hosted`) with `cancel-in-progress: false`, which does *not* queue unbounded — a group holds one running + one pending, and a third run cancels the pending one, so five PRs ended up with no `gate` check and became unmergeable. #760 fixed that with per-PR groups + host flock; the re-run/cancel edge is the cost of that fix.
...
|---------|---------------------|--------------------|
| **OpenCode** | `CASEIN_OPENCODE_DEFAULT_MODEL=<provider>/<model>` | `opencode/gpt-5.6-luna` |
...
| Paste/submit needs "double-Enter" folklore | Do **not** double-Enter yourself. `terminal_paste_agent_text` (`submit: true`) pastes without Enter, settles, then `PaneSubmit` presses Enter and retries once; `terminal_send_command` / `terminal_send_agent_command` confirm the same way. Trust `delivery`/`submitted` on the response. Pass explicit `pane` on paste for fleet workers without agent_pair. |
