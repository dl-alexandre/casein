defmodule CaseinPreviewBrowserTest do
  ...
  test "session persists health from asynchronous preview events" do
    ...
    event =
      {:preview_signal, "casein:preview:dom_loaded",
    ...
    assert %Health{
             state: :dom_loaded,
             dom_loaded: true,
             last_event_type: "casein:preview:dom_loaded",
             last_event_at: 123
           } = observed.health
  end
  ...
  test "playwright sidecar drives a real browser runtime" do
      ...
      <script>
        (() => {
          const emit = (type, extra = {}) => {
            window.dispatchEvent(new CustomEvent("casein:preview:signal", {
              detail: {
          ...
          emit("casein:preview:bridge_ready")
          document.addEventListener("DOMContentLoaded", () => {
            emit("casein:preview:dom_loaded")
            setTimeout(() => emit("casein:preview:live_socket_connected", {connected: true}), 0)
          })
        })()
    ...
    assert_receive {:preview_browser, ^browser_id,
                    {:preview_signal, "casein:preview:bridge_ready", bridge_payload,
    ...
    assert_receive {:preview_browser, ^browser_id,
                    {:preview_signal, "casein:preview:dom_loaded", _dom_payload,
                     %Health{dom_loaded: true}}}

    assert_receive {:preview_browser, ^browser_id,
                    {:preview_signal, "casein:preview:live_socket_connected", _socket_payload,
  ...
  test "real preview_bridge.js signals LiveView health through the Playwright sidecar" do
    ...
    url = "file://#{page_path}?casein_preview=1"
