defmodule CaseinMob.FilesScreen do
  ...
  def handle_info({:vt_host, host}, socket) when is_pid(host) do
    CaseinMob.DeviceBridge.put_host(host)
  ...
  def handle_info({:tap, :home}, socket),
    do: {:noreply, Mob.Socket.push_screen(socket, CaseinMob.HomeScreen)}
  ...
  defp request_list(socket, path) do
    case CaseinMob.DeviceBridge.host() do
      {:ok, host} ->
        ref = make_ref()
  ...
  defp open_entry(socket, %{name: name}) do
    ...
    Mob.Socket.push_screen(socket, CaseinMob.FileViewerScreen, %{path: path})
  end
