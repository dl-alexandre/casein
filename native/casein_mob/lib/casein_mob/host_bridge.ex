defmodule CaseinMob.HostBridge do
  @moduledoc """
  Host-side **Model-B-over-the-wire**: run a real shell on the dev host and stream
  its VT bytes to a `CaseinMob.TerminalScreen` running on a device, over Mob
  ...
  `input/2` (written into the PTY).
      ...
      {:ok, b} = CaseinMob.HostBridge.start(:"casein_mob_android_xxx@127.0.0.1")
      CaseinMob.HostBridge.input(b, "ls\\n")
  ...
  def init({device_node, opts}) do
    ...
    send({CaseinMob.DeviceBridge, device_node}, {:vt_host, self()})
