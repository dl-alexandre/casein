defmodule CaseinMob.MobileDogfoodFlowTest do
  ...
  test "first-run pairing through dashboard supervision and recovery" do
    ...
    pairing =
      ...
      |> mount_screen()
      |> render_info(
        {:change, :code, pairing_code("https://casein.test", "token", @workspace_id)}
      )
    ...
    assert SessionConfig.pairing() == {:ok, "https://casein.test", "token"}
