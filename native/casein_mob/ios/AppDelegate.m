...
#define MOB_PAIRING_CODE_MAX_BYTES 4096
...
static NSString* MobNotificationJSONFromReviewURL(NSURL* url) {
    if (![url.scheme isEqualToString:@"casein"] || ![url.host isEqualToString:@"review"]) {
        return nil;
    }
...
static NSString* MobNotificationJSONFromPairURL(NSURL* url) {
    if (![url.scheme isEqualToString:@"casein"] || ![url.host isEqualToString:@"pair"]) {
        return nil;
    }
    ...
    if (hasPath) {
        NSString* rawPath = components.percentEncodedPath;
        if (![rawPath hasPrefix:@"/"] || rawPath.length <= 1 ||
            [[rawPath substringFromIndex:1] containsString:@"/"]) {
            return nil;
        }
        code = [[rawPath substringFromIndex:1] stringByRemovingPercentEncoding];
        ...
        if ((![item.name isEqualToString:@"code"] &&
             ![item.name isEqualToString:@"pairing_code"]) ||
            item.value.length == 0) {
            return nil;
        }
        code = item.value;
    ...
    if (code.length == 0 ||
        [code lengthOfBytesUsingEncoding:NSUTF8StringEncoding] > MOB_PAIRING_CODE_MAX_BYTES) {
        return nil;
    }
...
- (BOOL)application:(UIApplication*)app
    ...
    const char* appModule = mob_app_module();
#ifdef CASEIN_RUNTIME_OVERLAY_GUARD
    CaseinRuntimeOverlayState runtimeState = CaseinRuntimeOverlayPrepare(appModule);
    NSLog(@"[CaseinRuntime] source=%s", CaseinRuntimeOverlayStateName(runtimeState));
    if (runtimeState == CaseinRuntimeOverlayStateBlocked) {
        mob_set_startup_error("Signed runtime verification failed — reinstall the reviewed build");
        return YES;
    }
#endif

    pthread_t t;
    if (pthread_create(&t, NULL, beam_thread, (void*)appModule) != 0) {
        mob_set_startup_error("BEAM could not start");
        return YES;
    }
    pthread_detach(t);
