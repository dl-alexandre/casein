// build.zig — Android native build for casein_mob.
//
...
//     Output: libcasein_mob.so installed to
//     android/app/src/main/jniLibs/<abi>/libcasein_mob.so so
...
pub fn build(b: *std.Build) void {
    ...
    // Per-NIF static flag for the generated driver_tab_android.zig, which gates
    // each subset-arch project NIF on `build_options.<module>_static`. mob_dev's
    // project_nif_zig_args emits `-Dghostty_vt_static=true` only on the arches
    // where the NIF + its `:extra_static_libs` archive is present (arm64-v8a
    // here); other ABIs leave it false and the driver_tab row is gated off.
    const ghostty_vt_static = b.option(bool, "ghostty_vt_static", "ghostty_vt VT NIF statically linked into this ABI") orelse false;
        ...
        const obj = if (std.mem.endsWith(u8, spec.source, ".zig")) blk: {
            ...
            const build_opts =
                if (std.mem.eql(u8, spec.name, "mob_beam")) opts: {
                    ...
                    o.addOption(bool, "ghostty_vt_static", ghostty_vt_static);
                    break :opts o;
                } else null;
