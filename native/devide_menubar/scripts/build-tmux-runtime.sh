...
OUTPUT_DIR="${1:-native/devide_menubar/build/tmux-runtime}"
mkdir -p native/devide_menubar/build
build_root="$(cd native/devide_menubar/build && pwd -P)"
case "/$OUTPUT_DIR/" in
  */../*|*/./*) echo "tmux output may not contain dot traversal" >&2; exit 1 ;;
esac
case "$OUTPUT_DIR" in
  "$build_root"/*) ;;
  native/devide_menubar/build/*) OUTPUT_DIR="$(pwd)/$OUTPUT_DIR" ;;
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/devide-tmux.XXXXXX")"
