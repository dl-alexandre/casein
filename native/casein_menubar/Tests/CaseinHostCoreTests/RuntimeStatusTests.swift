...
@Suite struct HostPathsTests {
    ...
    @Test func derivesContractPathsFromEnvironment() throws {
        let paths = try #require(
            HostPaths.detect(
                environment: [
                    "CASEIN_RELEASE_ROOT": "/opt/casein/release",
        ...
        #expect(paths.caseinBinary.path == "/opt/casein/release/bin/casein")
    ...
    @Test func reusesLegacyDataDirWhenItAlreadyExists() throws {
        let home = FileManager.default.temporaryDirectory
            .appending(path: "casein-host-home-\(UUID().uuidString)")
        let legacy = home.appending(path: "Library/Application Support/Casein")
    ...
    @Test func environmentOutranksPersistedChoice() throws {
        ...
        let overridden = try #require(
            HostPaths.detect(
                environment: ["CASEIN_RELEASE_ROOT": "/env/release"], defaults: defaults,
                bundleResources: nil))
    ...
    @Test func embeddedReleaseOutranksPersistedChoice() throws {
        ...
        let resources = FileManager.default.temporaryDirectory
            .appending(path: "casein-bundle-\(UUID().uuidString)")
