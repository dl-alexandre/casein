...
///
/// Gated on CASEIN_RELEASE_ROOT so `swift test` stays green without a
/// release. Run it with:
///
...
@Suite(
    .enabled(
        if: ProcessInfo.processInfo.environment["CASEIN_RELEASE_ROOT"]?.isEmpty == false,
        "requires CASEIN_RELEASE_ROOT pointing at a desktop-capable release"
    ),
    .serialized
)
struct LifecycleIntegrationTests {
    private func makePaths() throws -> HostPaths {
        let releaseRoot = URL(
            filePath: ProcessInfo.processInfo.environment["CASEIN_RELEASE_ROOT"]!
        ).standardizedFileURL
        try #require(
        ...
        let dataDir = FileManager.default.temporaryDirectory
            .appending(path: "casein-host-e2e-\(UUID().uuidString)")
    ...
    @Test func fullStartStopRestartLoop() async throws {
        ...
        let releaseNode = "casein_lifecycle_e2e_\(UInt32.random(in: 0..<UInt32.max))"
