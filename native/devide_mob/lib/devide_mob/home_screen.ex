defmodule DevideMob.HomeScreen do
  @moduledoc "Landing screen for DevideMob."
  use Mob.Screen
  ...
  def handle_info({:tap, :theme_light}, socket) do
    Mob.Theme.set(DevideMob.Theme.light())
  ...
  def handle_info({:tap, :open_text}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, DevideMob.TextScreen)}
  end

  def handle_info({:tap, :open_list}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, DevideMob.ListScreen)}
  end

  def handle_info({:tap, :open_dice}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, DevideMob.DiceScreen)}
  end

  def handle_info({:tap, :open_webview}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, DevideMob.WebViewScreen)}
  end

  def handle_info({:tap, :open_audio}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, DevideMob.AudioScreen)}
  end

  def handle_info({:tap, :open_storage}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, DevideMob.StorageScreen)}
  end
...
/>)
  ...
  defp theme_to_module(:light), do: DevideMob.Theme.light()
  defp theme_to_module(:dark), do: DevideMob.Theme.dark()
  defp theme_to_module(_), do: DevideMob.Theme.dark()
