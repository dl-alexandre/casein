defmodule DevideMob.Theme do
  @moduledoc """
  DevIDE's brand theme for the mobile companion.

  The palette is the web cockpit's (assets/css/app.css) converted from OKLCH to
  sRGB, so a workspace looks like the same product on the phone as in the
  browser: indigo primary on blue-tinted near-black surfaces.

  Applied at boot by `use Mob.App, theme: DevideMob.Theme` in `DevideMob.App`.
  `light/0` is the daylight counterpart — same hues, inverted surfaces — used by
  the theme switcher on the demo home screen.

  Semantic *status* colors (running / needs action / failed / done) are not
  theme tokens; they live in `DevideMob.UI` next to the components that use
  them.
  """
