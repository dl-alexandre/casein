defmodule DevideMob.StorageScreen do
  use Mob.Screen
  ...
  def render(assigns) do
    ...
    <Scroll background={:background}>
      <Column background={:background} padding={:space_lg}>
        ...
        <Spacer size={16} />
        <Button text="Write test file" background={:primary} text_color={:on_primary} padding={:space_md} fill_width={true} on_tap={tap_write} />
        <Spacer size={12} />
        <Button text="List documents" background={:surface} text_color={:on_surface} padding={:space_md} fill_width={true} on_tap={tap_list} />
        <Spacer size={12} />
        <Button text="Delete test file" background={:surface} text_color={:on_surface} padding={:space_md} fill_width={true} on_tap={tap_delete} />
        <Spacer size={20} />
  ...
  defp file_list(files) do
    items = Enum.map(files, fn f ->
      %{type: :text, props: %{text: Path.basename(f), text_size: :sm, text_color: :on_surface, padding: 4}, children: []}
    end)
    %{type: :column, props: %{}, children: items}
  end

  def handle_info({:tap, :write}, socket) do
    path = Path.join(Mob.Storage.dir(:documents), "devide_mob_test.txt")
    case Mob.Storage.write(path, "DevideMob storage test\n#{DateTime.utc_now()}") do
  ...
  def handle_info({:tap, :list}, socket) do
    case Mob.Storage.list(:documents) do
      {:ok, files}    -> {:noreply, Mob.Socket.assign(socket, files: files, status: "#{length(files)} file(s)")}
      {:error, posix} -> {:noreply, Mob.Socket.assign(socket, status: "list error: #{posix}")}
    end
  end
