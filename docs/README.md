...
| [`subsystems/mobile_action_center.md`](subsystems/mobile_action_center.md) | What a paired phone can do: the card types the observer projects (including `run_failed`), the three kinds of server-authored action spec (navigation, run mutation, agent instruction), and the `agent_instruction` path that pastes a bounded, audited prompt into a workspace's agent pane. |
| [`subsystems/push_notifications.md`](subsystems/push_notifications.md) | OS push fan-out (APNs/FCM) for session alerts and high-priority mobile cards — the offline counterpart to the live channel that reaches a backgrounded `devide_mob` app: native token acquisition, channel handoff, in-memory token registry, dispatcher, platform-routing providers, and the `DEV_IDE_*` runtime config to go live. |
