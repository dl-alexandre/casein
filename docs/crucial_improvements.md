...
| Field | Detail |
|-------|--------|
| **Source** | `lib/casein/loops/` (`Driver`, `Sandbox.Git`, `Runner`); absent from `docs/product.md`, `docs/architecture.md` |
| **Gap** | `Casein.Loops.Sandbox.Git` creates disposable worktrees, `git apply`s LLM-generated diffs, and runs `mix compile` + `mix test` with **no** `Casein.Policy` gate, **no** `Audit.emit_decision/2`, and **no** run-ledger event. A configured generator (`config :casein, Casein.Loops, generator: …`) can mutate and execute code off the request path via `Loops.Runner.start/2`. |
| **Invariant** | **FP-1** (execution authority server-side, recorded), **FP-10** (reviewable evidence), product §13 rule 1 |
| **Verify** | `test/casein/loops/driver_test.exs` exercises stubs only; grep `lib/casein/loops/` for `Policy` / `Audit` → zero hits |
...
| **Verify** | `test/casein/terminals/mode_policy_test.exs`, `test/casein_web/channels/terminal_channel_test.exs` (gate behavior when flag disabled) |
...
| **Status (2026-06-29)** | ✅ **CLOSED.** `Policy.can_use_raw_terminal?/1` and `Terminals.ModePolicy.raw_terminal_allowed?/2` now default to local host + `:manual` workspace mode. `config/runtime.exs` exposes `CASEIN_RAW_TERMINAL_EVERYWHERE=1|true|yes` as the explicit prod opt-in for raw-everywhere. Regression coverage lives in `test/casein/policy_test.exs`, `test/casein/terminals/boundary_test.exs`, and `test/casein/terminals/mode_policy_test.exs`. |
| **Addendum (2026-07-02)** | The mode-gate fix above didn't fully close the door: `Casein.Policy.WorkspaceMode`'s default fallback was still `:review`, not `:manual` — so a workspace with no persisted/config-pinned mode silently landed in `:review` and raw terminal (and split-screen) stayed unreachable with zero user-facing feedback (`do_split/2` in `show.ex` just returned `{:noreply, socket}` on deny). Fixed by flipping the default to `:manual` (`lib/casein/policy/workspace_mode.ex`). |

### P3 — Agent write and proposal apply permanently denied

| Field | Detail |
|-------|--------|
| **Source** | `lib/casein/policy.ex` (`:not_implemented`, `:agent_write_locked`), `docs/state_machines.md` mode table, `docs/architecture.md` authority map |
...
| **Verify** | `test/casein/policy_test.exs` ("can_apply_proposal? is always denied"), `test/casein/audit_test.exs` |
...
| **Status (2026-07-02)** | ✅ **CLOSED — option (a), the reviewed unlock flow.** `can_apply_proposal?/1` is real logic (operator + `:manual` mode + not isolation-blocked), backing a new `Casein.ProposalApply` write path (shells to `git apply`, atomic across a multi-file patch) and a Proposals tab in the LiveView — the read-only `Casein.Proposals` discover/parse/analyze modules were deliberately left untouched (`test/casein/proposals_no_apply_test.exs` rewritten, not deleted, to assert that boundary). `can_enable_agent_write?/1` is real logic too: `:manual` mode plus an explicit, time-boxed, human-granted unlock (`Workspaces.grant_agent_write_unlock/3`, revocable with no mode/isolation gate — the kill switch), backing `Casein.Proposals.AutoApply`, which polls a completed review-agent run (`Agents.Run.state/1`, not `subscribe/1` — that's single-subscriber) and auto-applies only a `:clean`-risk, non-`test/`-touching proposal, gated again by a deployment-wide config kill switch (default off). The anti-gaming content veto is a direct, deliberate port of the deleted `Loops` subsystem's `analyze_diff/1` signal (see `docs/crucial_improvements.md` Safety & Policy P1) — this is the informed revival that removal's rationale anticipated, with `Policy` + `Audit` + run-ledger coverage on every branch from day one, unlike the original. `Casein.Agents.Run` itself still cannot run an arbitrary command, send a prompt, or apply a patch directly — the write path is entirely in the new, separate modules. **Residual:** the allowlist has no real `output_kind: :proposal` `ReviewCommand` yet (`docs/hardening.md` / `AGENTS.md` still only lists the diagnostic `opencode-version` entry — no write-free upstream OpenCode review subcommand exists), so `AutoApply` is fully wired but has nothing to trigger on in practice until that follow-up lands. |
...
| Field | Detail |
|-------|--------|
| **Source** | `lib/casein_web/plugs/forward_auth.ex` moduledoc (OPTIONS + `/site.webmanifest` bypass), `lib/casein/application.ex` (`assert_forward_auth_bind!/0`) |
...
| **Verify** | `lib/casein/application.ex:115-130`; enable `CASEIN_FORWARD_AUTH=1` without loopback bind in dev → raises |
...
| Field | Detail |
|-------|--------|
| **Source** | `lib/casein_web/live/workspace_live/show.ex` (~L131, ~L308), `test/casein_web/live/workspace_live_test.exs`, `docs/audit_remote.md` CC-6 (awareness) |
...
| **Verify** | `mix test test/casein_web/live/workspace_live_test.exs` (cross-host flash assertion) |
...
| Field | Detail |
|-------|--------|
| **Source** | `lib/casein/terminals/attachment.ex` L62–63 |
...
| Field | Detail |
|-------|--------|
| **Source** | `docs/audit_local.md` row 5, `lib/casein/terminals/session.ex` (`@buffer_bytes`), `test/casein/terminals/session_test.exs` |
...
| **Verify** | `test/casein/terminals/session_test.exs` ("replays buffered output…"); tmux capture test for restart path |
...
| Field | Detail |
|-------|--------|
| **Source** | `docs/terminal_mcp.md` §Agent pairing, `lib/casein/agents/terminal_tools.ex` (`terminal_send_agent_*`), `AGENTS.md` friction table |
...
| **Verify** | `scripts/verify_agent_pairing.sh`; `test/casein_web/api/terminal_mcp_test.exs` |
...
| **Verify** | `test/casein_web/controllers/api/preview_mcp_controller_test.exs` |
...
| Collaborator role (between viewer and owner) | `lib/casein/policy.ex` `workspace_role/1` comment | Only admin/owner/viewer today |
...
`lib/casein/loops/`, `lib/casein/loops.ex`, tests, `Policy.can_run_loop?/1`,
...
# Session durability
mise exec -- mix test test/casein/terminals/session_test.exs test/casein/terminals/tmux_janitor_test.exs
