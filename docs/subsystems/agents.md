...
| `DevIDE.Agents.AuthProfile` | `lib/dev_ide/agents/auth_profile.ex` | Resolve opt-in workspace-scoped Claude/Codex auth homes from `~/.devide/agent-auth/<workspace>/<runtime>` directory presence. Missing dirs keep global provider auth. |
...
3. `PaneEnv` builds the `DEVIDE_*` env map (`DEV_IDE_API_TOKEN`,
   ...
   `~/.devide/agent-auth/<workspace>/<runtime>`, `PaneEnv` also injects
...
- **Provider auth profiles are opt-in by directory presence.** Do not persist
  provider secrets in `workspace_records` or manager metadata. Create
  `~/.devide/agent-auth/<workspace>/claude` or `codex` with
  `devide agent auth login <workspace> <runtime>` when a workspace should use a
  separate provider subscription; delete the directory to return to global auth.
  Use `devide agent auth status [workspace] [runtime]` or
  `devide agent auth list` to audit which workspaces have opt-in profiles.
