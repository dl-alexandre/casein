...
| `Casein.Desktop.NativeAgentLaunch` | `lib/casein/desktop/native_agent_launch.ex` | Provides one prepare/start transaction across isolated worktree creation, provider MCP materialization, ConPTY session start, runtime reporting, and explicit clean-only finish cleanup. |
