...
- **Audit is append-only.** There is no update/delete API. A rare duplicate
  (e.g. raw-reconnect dedup scrolling out of `Boundary`'s 20-event lookback)
...
- [`../architecture.md`](../architecture.md) — FP-10, the "Event plane" table
  (durable vs. realtime), the adapter pattern, and the authority map (which
  actions produce which ledger/audit records).
