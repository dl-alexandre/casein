...
| Module | File | Role |
|--------|------|------|
| `ExecCtl` | `dev_ide_core/lib/exec_ctl.ex` | Boundary root (`deps: []`). erlexec streaming + static allowlists live here; Casein audit/policy/argv-building stays in `Casein.Commands.*`. |
...
| Module | File | Role |
|--------|------|------|
| `GitCtl` | `dev_ide_core/lib/git_ctl.ex` | Boundary root (`deps: []`). Subprocess parsing + optional ETS caching live here; Casein workspace policy and mutating git ops stay in `Casein.Git.*`. |
...
| Module | File | Role |
|--------|------|------|
| `McpCtl` | `dev_ide_core/lib/mcp_ctl.ex` | Boundary root (`deps: []`). Schema fragments shared across Casein's agent-facing tools; the tool *implementations* stay in `Casein.Agents.*Tools`. |
...
| `McpCtl.Schema` | `dev_ide_core/lib/mcp_ctl/schema.ex` | Workspace-scoping JSON-schema fragments: `workspace_id_param/1` (`:terminal` vs `:preview` variant), `workspace_path_param/0`, `workspace_object/1`, `merge_workspace_properties/2`. Encodes Casein workspace-id / folder-attachment wording. |
