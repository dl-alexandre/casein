...
| Module | File | Role |
|--------|------|------|
| `ExecCtl` | `dev_ide_core/lib/exec_ctl.ex` | Boundary root (`deps: []`). erlexec streaming + static allowlists live here; DevIDE audit/policy/argv-building stays in `Casein.Commands.*`. |
...
| Module | File | Role |
|--------|------|------|
| `GitCtl` | `dev_ide_core/lib/git_ctl.ex` | Boundary root (`deps: []`). Subprocess parsing + optional ETS caching live here; DevIDE workspace policy and mutating git ops stay in `Casein.Git.*`. |
...
| Module | File | Role |
|--------|------|------|
| `McpCtl` | `dev_ide_core/lib/mcp_ctl.ex` | Boundary root (`deps: []`). Schema fragments shared across DevIDE's agent-facing tools; the tool *implementations* stay in `Casein.Agents.*Tools`. |
