...
Source: Addy Osmani, "How modern browsers work." Mapped onto casein's terminal
...
| L5 | rAF-coalesced paint | **Shipped, default-OFF** | New `paintCanvasCellsCoalesced` collapses bursty `onRenderCells` calls to one paint/frame. Gated behind `data-coalesce="raf"` / `localStorage["casein:terminal-coalesce"]="raf"`, **and** only active in the already-opt-in canvas branch. Inert until both flags are set. |
...
1. `localStorage["casein:terminal-renderer"] = "canvas"` (enable canvas).
2. `localStorage["casein:terminal-coalesce"] = "raf"` (enable coalescing).
