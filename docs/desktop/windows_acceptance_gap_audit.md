...
| Corrupted state recovery | Release-file repair, SQLite pre-migration corruption rejection, malformed settings/runtime cleanup, and fail-closed plus verified-package recovery for malformed `current.json` are package-smoked | Exercise interrupted staging with fail-closed/repair outcomes |
