...
| Corrupted state recovery | Release-file repair, SQLite pre-migration corruption rejection, malformed settings/runtime cleanup, malformed `current.json` recovery, and bounded interrupted-staging cleanup are package-smoked | Capture these recoveries on a production-signed disposable host |
