...
**Snapshot:** 2026-08-02 after PRs #509, #516, and #521
...
PR #509 has since landed support-bundle allowlisting, #516 landed stable
launch-at-sign-in lifecycle handling, and #521 landed package-path acceptance
canaries. Native provider worktree launch remains separately owned.
...
| Corrupted state recovery | Release-file repair, SQLite pre-migration corruption rejection, malformed settings/runtime markers, malformed `current.json`, and interrupted install staging are package-smoked | Capture the same recovery on a production-signed disposable host |
