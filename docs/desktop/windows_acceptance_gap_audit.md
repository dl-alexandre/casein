...
| Corrupted state recovery | Release-file repair, SQLite pre-migration corruption rejection, malformed-settings fallback, and stale/corrupt runtime-marker cleanup are package-smoked | Exercise malformed `current.json` and interrupted staging with fail-closed/repair outcomes |
