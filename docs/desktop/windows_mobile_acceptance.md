...
The Windows package smoke also writes malformed desktop settings, runtime
markers, installed `current.json`, and an interrupted install staging tree into
its disposable LocalAppData root. It verifies safe tray fallback, stale runtime
cleanup, replacement of invalid installed-release state, and bounded staging
cleanup before a verified reinstall. This remains repository evidence only;
production-signed disposable-host execution belongs to #376.
