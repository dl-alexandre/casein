...
| `terminal_paste_agent_text` | Paste literal/multiline text via tmux paste buffer; optional `pane` skips agent_pair; `submit` confirms Enter; refuses agents view / menus unless `allow_non_conversation` | `text`\*, `submit`, `confirm`, `pane`, `session`, `allow_non_conversation` | `paste_agent_text/1` |
...
| `terminal_send_command` | Type command + Enter into a targeted pane; confirms submit (one retry); refuses agents view / menus unless `allow_non_conversation` | `session`\*, `command`\*, `pane`, `confirm`, `allow_non_conversation` | `send_command/1` |
