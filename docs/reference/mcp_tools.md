...
| `terminal_paste_agent_text` | Paste literal/multiline text via tmux paste buffer; optional `pane` skips agent_pair; `submit` confirms Enter; refuses agents view / menus unless `allow_non_conversation`; receipt includes `input_buffer` | `text`\*, `submit`, `confirm`, `pane`, `session`, `allow_non_conversation` | `paste_agent_text/1` |
